...
#名詞解釋:word(在程式中常以w出現)就是組成16 byte block(在程式中常以state出現)的四根column(柱子)的其中一根，
#所以是4 bytes
S_BOX = [
...
def add_round_key(state, round_key):
    return [(b ^ k) & 0xFF for b, k in zip(state, round_key)]
...
def inv_mix_single_column(col):
    a0, a1, a2, a3 = col
    r0 = gf_mul(a0, 0x0e) ^ gf_mul(a1, 0x0b) ^ gf_mul(a2, 0x0d) ^ gf_mul(a3, 0x09)
    r1 = gf_mul(a0, 0x09) ^ gf_mul(a1, 0x0e) ^ gf_mul(a2, 0x0b) ^ gf_mul(a3, 0x0d)
    r2 = gf_mul(a0, 0x0d) ^ gf_mul(a1, 0x09) ^ gf_mul(a2, 0x0e) ^ gf_mul(a3, 0x0b)
    r3 = gf_mul(a0, 0x0b) ^ gf_mul(a1, 0x0d) ^ gf_mul(a2, 0x09) ^ gf_mul(a3, 0x0e)
    return [r0 & 0xFF, r1 & 0xFF, r2 & 0xFF, r3 & 0xFF]
